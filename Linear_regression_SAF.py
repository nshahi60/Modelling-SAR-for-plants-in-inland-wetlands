...
for i, file_path in enumerate(csv_files):
    ...
    # Select all relevant columns
    selected_columns_all = ['Species richness', 'SatAreaFrac', 'SatAreaFrac_20', 'SatAreaFrac_40', 'SatAreaFrac_60']
    df_selected_all = df[selected_columns_all]

    # Impute missing values using the mean strategy
    df_selected_all = df_selected_all.apply(lambda col: col.fillna(col.mean()) if col.name != 'SatAreaFrac_60' else col)
    
    # Drop rows where 'Species richness' is NaN
    df_selected_all = df_selected_all.dropna(subset=['SatAreaFrac_60'])

    # Scatter plot for 'Species richness' and 'Area (km2)'
    sns.scatterplot(x='SatAreaFrac_60', y='Species richness', data=df_selected_all, ax=axes[i])

    # Linear Regression for 'Species' and 'Area (km2)'
    X_all = df_selected_all[['SatAreaFrac_60']]
    y_all = df_selected_all['Species richness']
    model_all = LinearRegression()
    model_all.fit(X_all, y_all)
    y_pred_all = model_all.predict(X_all)
    ...
    axes[i].annotate(equation, xy=(0.50, 0.85), xycoords='axes fraction', ha='left', va='center', fontsize=22)

    # Set title and labels
    axes[i].set_title(f'{country_name}', fontsize=21)
    axes[i].set_xlabel('Saturated Area Fraction_60', fontsize = 19)
...
plt.tight_layout()
output_file = '/home/1561626/plots/Linear_regression_SR_vs_SAF60.png'
