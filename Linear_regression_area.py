...
for i, file_path in enumerate(csv_files):
    ...
    country_name = os.path.basename(file_path).split('.')[0].strip()
    ...
    # Select all relevant columns
    selected_columns_all = ['Area (km2)', 'Species richness']
    df_selected_all = df[selected_columns_all]
    ...
    sns.scatterplot(x='Area (km2)', y='Species richness', data=df_selected_all, ax=axes[i])
    ...
    # Calculate R-squared, MAE and p-value
    r2_all = r2_score(y_all, y_pred_all)
    slope, intercept, r_value, p_value, std_err = linregress(X_all.values.flatten(), y_all.values)
    error=mae(y_all, y_pred_all)
    ...
    axes[i].set_xlabel('Area (sq.km)', fontsize = 19)
    axes[i].set_ylabel('Species Richness', fontsize=19)
    axes[i].tick_params(axis='x', labelsize=14)
    axes[i].tick_params(axis='y', labelsize=14)
    
    #residual plot
    axes1[i].scatter(X_all, residuals)
    axes1[i].axhline(y=0, color='black', linewidth=3)
    axes1[i].set_title(f'{country_name}', fontweight="bold", fontsize=21)
    axes1[i].set_xlabel('Area (sq.km)', fontsize=19)
    axes1[i].set_ylabel('Residuals', fontsize=19)
    axes1[i].tick_params(axis='x', labelsize=14)
    axes1[i].tick_params(axis='y',labelsize=14)
...
error_df.to_csv(f"/home/1561626/MAE_p-value/Linear_Area_SR_error.csv", index=False)

# Adjust layout and save the figure
fig.tight_layout()
fig1.tight_layout()
fig.savefig('/home/1561626/plots/Linear_regression_SR_vs_Area.png')
fig1.savefig('/home/1561626/plots/residual_linear_SR_Area.png')
#plt.show()
